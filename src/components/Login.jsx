...
const Login = () => {
  ...
  const loginUser = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:3010/loginuser', {
        email,
        password
      });
  ...
  return (
      <div className=" grid grid-cols-1 md:grid-cols-6 h-100v justify-center bg-gray-200">
         ...
         <div className="col-span-2 bg-linear-9 h-100v p-5 my-auto  max-[670px]:hidden">
          <Image data-aos="zoom-in-up" className=" margin-t10" src='/assetku/login.png' />
         </div>
        <div className="col-span-4 grid grid-cols-6 gap-1 w-full bg-white text-center  h-100v">
