...
import ModalTambah from "./InfografikModalTambah";
import ModalDelete from "./InfografikModalDelete";
...
export default function Iklanlist() {
  ...
  const getIklanSearch = async () => {
    const res = await api_url_satuadmin.get("api/opendata/infografik/admin");
  ...
  const rowsku = Array.isArray(rowsFiltered)
    ? rowsFiltered.map((row, index) => ({
        ...
        no: index + 1,
        ...row,
        penyusun_nama_topik:`${row.penyusun ?? ''} ${row.nama_topik ?? ''}`
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        return (
          <div style={{ textAlign: "center", width: "100%" }}>
            {row.presignedUrl_a_tumb && (
              <Image
                src={row.presignedUrl_a_tumb}
            ...
            {row.presignedUrl_b_tumb && (
              <Image
                src={row.presignedUrl_b_tumb}
                className="rad15 px-3"
                style={{ maxWidth: 130, objectFit: "contain" }}
              />
            )}
            ...
            {row.presignedUrl_c_tumb && (
              <Image
                src={row.presignedUrl_c_tumb}
                className="rad15 px-3"
                style={{ maxWidth: 130, objectFit: "contain" }}
              />
            )}
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div className="gap-2 p-2">
          <Link to={`/Opendata/Infografik/Update/${params.row.id_infografik}`}>
          ...
          <ModalDelete id={params.row.id_infografik} name={params.row.title} />
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="bg-slate-100 w-full max-h-screen sm:pt-0 max-[640px]:mt-12">
      <NavSub title="Open Data Infografik" />
      ...
      <div className="col-span-3 grid grid-cols-1 md:grid-cols-6 gap-4">
        <div className="col-span-3">
          <p className="font-semibold text-gray-300 flex pt-2 mt-2 mx-3 mb-0">
            ...
            /{" "}
            <NavLink to="#" className="textsize11 text-silver-a mx-2 flex">
              <MdDataset className="mt-1" /> Opendata Infografik
            </NavLink>
          </p>
        </div>
      ...
      <div className="drop-shadow-lg overflow-auto mb-9 p-2">
        <section className="block   py-3 rad15 shaddow1 bg-white px-2">
          <Container fluid>
            <Tabs
            ...
            >
              ...
              <Tab eventKey="aktivitas" title="Aktivitas">
                  <Activity kunci={'Open Data Infografik'}/>
              </Tab>
            </Tabs>
