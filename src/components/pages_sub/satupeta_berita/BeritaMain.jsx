...
export default function Iklanlist() {
  ...
  const getIklanSearch = async () => {
    const res = await axios.get(apiurl + "api/satupeta/map_berita");
      ...
      renderCell: (params) => {
        ...
        return (
          <div style={{ textAlign: "center", width: "100%" }}>
            ...
            {row.title && (
              <p className="textsize11 text_justif font_weight700" style={{color:"#0B486B"}}>
                {row.title}
              </p>
            )}
            {row.sumber && (
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div className="gap-2 p-2">
          <Link to={`/Satupeta/Berita/Update/${params.row.title}`}>
          ...
          <ModalDelete id={params.row.id} name={params.row.title} />
        </div>
      ),
    },
  ];
        ...
        <div className="col-span-3">
          <p className="font-semibold text-gray-300 flex pt-2 mt-2 mx-3 mb-0">
            ...
            <NavLink to="#" className="text-link-sky mx-2 flex">
              <MdDataset className="mt-1" /> Satupeta Berita
            </NavLink>
          </p>
        </div>
