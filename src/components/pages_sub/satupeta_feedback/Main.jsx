...
export default function Iklanlist() {
  ...
  const getIklanSearch = async () => {
    const res = await api_url_satuadmin.get(`api/open-item/satupeta_feedback`);
    const data = res.data || [];
    setDatasetku(data);
  ...
  const handleDeleteMultiple = async () => {
    ...
    if (confirm.isConfirmed) {
      try {
        await api_url_satuadmin.post(`api/open-item/satupeta_feedback/delete`, {
          ids: selectedIds,
          admin: userloginadmin,
          jenis: "Satu Peta Feedback",
          komponen: "Delete Feedback Satu Peta"
        });
  ...
  return (
    <div className="bg-slate-100 w-full max-h-screen sm:pt-0 max-[640px]:mt-12">
      <NavSub title="Satu Peta Feedback" />
        ...
        <div className="col-span-3">
          <p className="font-semibold text-gray-300 flex pt-2 mt-2 mx-3 mb-0">
            ...
            <NavLink to="#" className="textsize10 text-silver-a mx-2 flex">
              <MdDataset className="mt-1" /> Satupeta Feedback
            </NavLink>
          </p>
        </div>
      </div>
