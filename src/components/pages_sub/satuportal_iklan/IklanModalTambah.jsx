...
function ModalTambahUser() {
  ...
  const saveIklan = async (e) => {
    ...
    try {
      await axios.post(apiurl + 'api/open-item/ekosistem-iklan/add', formData);
  ...
  const handle_step1 = (event) => {
    ...
    if (status.length < 3) {
      setvalidasi_status(true);
    } else {
      setvalidasi_status(false);
    }
    ...
    if (title.length >= 3 && status.length >= 3 && file) {
      nextStep();
    }
  };
    ...
    <>
        ...
        >
            ...
            <Modal.Header closeButton className="border-b ">
                <h4 className="text-sky-600 flex"><MdAddCircle  className="tsize-90 text-sky-600 mt-1"  />Tambah Satu Portal Iklan</h4>
