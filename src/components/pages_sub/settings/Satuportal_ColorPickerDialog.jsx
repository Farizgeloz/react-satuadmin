...
const ColorPickerDialog = ({ isOpen, onClose, rowData, onSave }) => {
  ...
  const getSatuportal_listSearch = async () => {
    try {
      const response = await api_url_satuadmin.get('openitem/site_ekosistem_setting');
  ...
  const handleSave = async () => {
    ...
    formData.append("jenis","Satu Portal Color");
    formData.append("komponen","Update Color Satu Portal");
    try {
      setLoading(true);
      ...
      await api_url_satuadmin.patch(`openitem/site_ekosistem_setting_update/${colors.id}`, formData, {
        headers: {
